# -*- coding: utf-8 -*-
"""
Created on Thu May 18 22:27:32 2017
...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if abs(steering) > max_steering_angle:
            raise ValueError, 'Exceeding the max steering angle'
        if distance < 0.0:
            raise ValueError, 'Moving backword is not valid'
            
        #apply noise
        steering = random.gauss(steering,self.steering_noise)
        distance = random.gauss(distance,self.distance_noise)
        
        bata = distance/self.length * tan(steering)
        
        if abs(bata)<0.001:
            self.x = self.x + cos(self.orientation)*distance
            self.y = self.y + sin(self.orientation)*distance
            self.orientation = (self.orientation+bata) % (2*pi)
        else:
            radius =  distance/bata
    ...
    def sense(self,add_noise=1): #do not change the name of this function
        ...
        for landmark in landmarks:
            realAngle = atan2(landmark[0]-self.y,landmark[1]-self.x)
            bearing = realAngle - self.orientation
